...
void usbportListen(void* arg)
{
    ...
    while(n <= 0 ){
    	printf("waiting..........\n");
    	n = recvfrom(sockfd, recv_mesg, MAXLINE, 0, (SA *) &cliaddr,&len);
    ...
    n =  sendto(sockfd, back_mesg, sizeof(back_mesg), 0, (SA *) &cliaddr, len);
    ...
    while(thread_flag)
    {
        ...
        {
        	//modify by cb
        	   nread= recvfrom(sockfd, usbbuf+total_read, MAXLINE, 0, (SA *) &cliaddr, &len);//读udp数据
//           nread = read(fdusb, usbbuf+total_read, 1024); //读USB串口
        ...
        for(i=0; i<total_read-7; i++)//找到数据包标志
        {
                ...
                && usbbuf[i+4] == 0x00 && usbbuf[i+5] == 0x00 && usbbuf[i+6] == 0x01)
            {
                ...
                {
                    nread= recvfrom(sockfd, mesg+total_read, MAXLINE, 0, (SA *) &cliaddr, &len);
                    total_read += nread;
                    //DEBUG printf("total_read=%d, i=%d\n", total_read, i);
                }
       ...
       if (*(start_p+10) == DATA)//内容包
        {
                    ...
                    {
                        nread= recvfrom(sockfd, mesg+total_read, MAXLINE, 0, (SA *) &cliaddr, &len);//读USB串口
                        total_read += nread;
                    }
